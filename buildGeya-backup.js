...
export default function buildGeya(slug, chapter, titleLineNumber) {
  ...
  fetch(`./root/${slug}_root-pli-ms.json`)
    .then(response => response.json())
    .then(data => (paliData = data))
    .then(() => {
      fetch(`./translation/${slug}_translation-en-sujato.json`)
        .then(response => response.json())
        .then(data => (transData = data))
        .then(() => {
          fetch(`./html/${slug}_html.json`)
            .then(response => response.json())
            .then(data => (htmlData = data))
            .then(() => {
                ...
                if (inAVerse) {
                  ...
                  if ((line === "0") & (subLine <= titleLineNumber - 1)) {
                    ...
                    html += `${openHtml}<span class="pli-lang">${
                      (line === "0") & (subLine === titleLineNumber)
                        ? `<span class="sutta-number">${verse}.&nbsp;</span>`
                        : ""
